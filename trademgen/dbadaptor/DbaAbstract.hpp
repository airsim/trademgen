#ifndef __FORECAST_DBA_DBAABSTRACT_HPP
#define __FORECAST_DBA_DBAABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const FORECAST::DbaAbstract& iDba) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            FORECAST::DbaAbstract& ioDba) {
  // Fill Dba object with input stream
  ioDba.fromStream (ioIn);
  return ioIn;
}

#endif // __FORECAST_DBA_DBAABSTRACT_HPP
